...
public class ArduinoController : MonoBehaviour {
	...
	public float oldGyroX = 0;
	public float oldGyroY = 0;
	public float oldGyroZ = 0;
	...
	void MoveObject(string[] arduinoData){
		if (arduinoData.Length == 7) {
			...
			float gDeltaX = gyroX - oldGyroX;
			float gDeltaY = gyroY - oldGyroY;
			float gDeltaZ = gyroZ - oldGyroZ;

			float newGyroX = transform.rotation.x + gDeltaX;
			float newGyroY = transform.rotation.y + gDeltaY;
			float newGyroZ = transform.rotation.z + gDeltaZ;

			camera.GetComponent<CameraController> ().unityRotate (gyroX, gyroY, gyroZ);
